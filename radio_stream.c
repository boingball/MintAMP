...
struct RadioStream {
     ...
     * keep the flag for diagnostics, but cleanup now follows the repro and
     * still calls SSL_free(). */
...
{
    /* Only detected memory corruption should hard-poison AmiSSL. Ordinary
     * TLS failures are logged, cleaned up, and the next HTTPS attempt is
     * allowed to build a fresh child-owned AmiSSL lifecycle. */
    return "HTTPS disabled after memory corruption; reboot before using HTTPS.";
}

/* Fatal TLS faults are counted and logged, but they do not disable HTTPS for
 * the app run. The child-owned path must instead clean up safely in the same
 * order as amissl_child_worker_repro.c, restore parent globals, return the
 * GUI to error/idle, and allow the next HTTPS attempt. */
...
void Radio_ReportTlsFault(const char *where)
{
    ...
    RADIO_DBG(printf("radio-tls: TLS fault %ld where=%s session=%lu url=\"%s\" -- cleaned up; HTTPS remains available for a fresh attempt\n",
        radio_tls_fault_count, where ? where : "",
        radio_poison_session_id, radio_poison_url[0] ? radio_poison_url : ""));
}

/* Hosts that triggered a fatal AmiSSL fault this run. Kept for diagnostics;
 * the production HTTPS path no longer blocks future attempts by host. */
...
void Radio_NoteTlsFaultHost(const char *host)
{
    ...
    RADIO_DBG(printf("radio-tls: host \"%s\" noted for TLS fault diagnostics count=%d\n",
...
static int radio_net_open_child(RadioStream *rs)
{
    ...
    if (!ctx->amisslMasterBase) {
        RADIO_DBG(printf("BEFORE CloseLibrary(bsdsocket) session=%lu base=%p (amisslmaster open fail)\n",
            rs->session_id, (void *)ctx->socketBase););
        CloseLibrary(ctx->socketBase);
        RADIO_DBG(printf("AFTER CloseLibrary(bsdsocket) session=%lu (amisslmaster open fail)\n", rs->session_id););
        ctx->socketBase = NULL;
        ctx->childOwned = 0;
    ...
    RADIO_DBG(printf("BEFORE OpenAmiSSLTags session=%lu task=%p SocketBase=%p AmiSSLMasterBase=%p ErrNoPtr=%p\n",
        rs->session_id, (void *)ctx->ownerTask, (void *)ctx->socketBase,
        (void *)ctx->amisslMasterBase, (void *)&ctx->errnoStore););
    if (OpenAmiSSLTags(AMISSL_CURRENT_VERSION,
                       ...
                       TAG_DONE) != 0) {
        RADIO_DBG(printf("AFTER OpenAmiSSLTags fail session=%lu SocketBase=%p AmiSSLMasterBase=%p AmiSSLBase=%p AmiSSLExtBase=%p ErrNoPtr=%p\n",
            rs->session_id, (void *)ctx->socketBase, (void *)ctx->amisslMasterBase,
            (void *)ctx->amisslBase, (void *)ctx->amisslExtBase, (void *)&ctx->errnoStore););
        RADIO_DBG(printf("BEFORE CloseLibrary(amisslmaster) session=%lu base=%p (OpenAmiSSLTags fail)\n",
            rs->session_id, (void *)ctx->amisslMasterBase););
        CloseLibrary(ctx->amisslMasterBase);
        RADIO_DBG(printf("AFTER CloseLibrary(amisslmaster) session=%lu (OpenAmiSSLTags fail)\n", rs->session_id););
        RADIO_DBG(printf("BEFORE CloseLibrary(bsdsocket) session=%lu base=%p (OpenAmiSSLTags fail)\n",
            rs->session_id, (void *)ctx->socketBase););
        CloseLibrary(ctx->socketBase);
        RADIO_DBG(printf("AFTER CloseLibrary(bsdsocket) session=%lu (OpenAmiSSLTags fail)\n", rs->session_id););
    ...
    RADIO_DBG(printf("AFTER OpenAmiSSLTags success session=%lu SocketBase=%p AmiSSLMasterBase=%p AmiSSLBase=%p AmiSSLExtBase=%p ErrNoPtr=%p\n",
        rs->session_id, (void *)ctx->socketBase, (void *)ctx->amisslMasterBase,
        (void *)ctx->amisslBase, (void *)ctx->amisslExtBase, (void *)&ctx->errnoStore););
    ctx->amiSslOpened = 1;
...
static void radio_net_close_child(RadioStream *rs)
{
    ...
    if (ctx->amiSslOpened && ctx->amisslBase) {
        RADIO_DBG(printf("BEFORE CloseAmiSSL session=%lu base=%p\n",
            rs->session_id, (void *)ctx->amisslBase););
        CloseAmiSSL();
    ...
    if (ctx->amisslMasterBase && closeChildBases) {
        RADIO_DBG(printf("BEFORE CloseLibrary(amisslmaster) session=%lu base=%p\n",
            rs->session_id, (void *)ctx->amisslMasterBase););
        CloseLibrary(ctx->amisslMasterBase);
    ...
    if (ctx->socketBase && closeChildBases) {
        RADIO_DBG(printf("BEFORE CloseLibrary(bsdsocket) session=%lu base=%p\n",
            rs->session_id, (void *)ctx->socketBase););
        CloseLibrary(ctx->socketBase);
    ...
    memset(ctx, 0, sizeof(*ctx));
    RADIO_DBG(printf("LIFECYCLE session=%lu child-close-done openamissl_count=%ld closeamissl_count=%ld active_ssl=%ld active_ctx=%ld open_socket=%ld playback_socket=%ld childOwned=0 parent_restored=1\n",
        rs->session_id, radio_openamissltags_count, radio_closeamissl_count,
        radio_active_ssl_count, radio_active_ssl_ctx_count, radio_open_socket_count,
        radio_playback_open_socket_count););
    if (locked) Radio_AmiSslUnlock();
...
static int radio_ssl_do_handshake(RadioStream *rs)
{
    ...
    for (tries = 0; tries < 150; tries++) {
        ...
        if (r == 1) {
            RADIO_DBG(printf("AFTER SSL_connect success session=%lu attempt=%d ret=%d ssl=%p ctx=%p fd=%ld\n",
        ...
        RADIO_DBG(printf("AFTER SSL_connect fail session=%lu attempt=%d ret=%d ssl_error=%d ssl=%p ctx=%p fd=%ld\n",
         ...
         * SSL_get_error()'s numeric code alone doesn't say. */
        {
            ...
            if (rs && radio_ssl_error_is_fatal(e)) {
                /* Fatal AmiSSL-level handshake failure. Record the first
                 * SSL_get_error()/ERR_get_error() details, stop this stream,
                 * and let the child-owned cleanup path follow
                 * amissl_child_worker_repro.c so the next HTTPS attempt can
                 * start from fresh per-child AmiSSL state. */
                ...
                RADIO_DBG(printf("radio-safety: TLS session fault reason=%s session=%lu\n", reason, rs->session_id));
                ...
                RADIO_DBG(printf("radio-tls: session=%lu handshake TLS fault reported -- repro-order cleanup will run\n", rs->session_id));
            }
...
static int radio_ssl_connect(RadioStream *rs)
{
    ...
    /* Match amissl_child_worker_repro.c: create a fresh SSL_CTX for each
     * HTTPS connection attempt instead of reusing one across reconnects. */
    if (!rs->ctx) {
        method = SSLv23_client_method();
        if (!method) { set_error(rs, "AmiSSL init failed"); return -1; }
        RADIO_DBG(printf("BEFORE SSL_CTX_new session=%lu method=%p SocketBase=%p AmiSSLBase=%p ErrNoPtr=%p\n",
            rs->session_id, (void *)method, (void *)SocketBase, (void *)AmiSSLBase,
            (void *)&rs->net.errnoStore););
...
static void radio_ssl_close_stream_mode(RadioStream *rs, RadioCloseMode mode)
{
    ...
    if (rs->ssl && !rs->sslFreed) {
            ...
            rs->sslStatePoisoned, Radio_IsTlsPoisoned(), Radio_IsMemoryPoisoned()));
        {
            ...
            SSL_free(rs->ssl);
            RADIO_DBG(printf("AFTER SSL_free session=%lu\n", rs->session_id););
            rs->ssl_free_count++;
    ...
    if (rs->ctx && !rs->ctxFreed) {
        ...
        SSL_CTX_free(rs->ctx);
        RADIO_DBG(printf("AFTER SSL_CTX_free session=%lu\n", rs->session_id););
...
/* Fallback final SSL_CTX free. The repro-matching path frees the per-attempt
 * SSL_CTX in radio_ssl_close_stream_mode(); this catches any early-exit path
 * that allocated a context but did not reach socket close. */
    ...
    while (sent < len && tries < 150) {
        ...
        if (rs->isSSL && rs->ssl) {
            ...
            if (r > 0) { sent += r; continue; }
            {
                ...
                /* Same fatal classification as SSL_connect()/SSL_read(): log
                 * the SSL_get_error()/ERR_get_error() result, stop this
                 * stream, then let repro-order cleanup run. */
...
static void radio_abort_current_socket(RadioStream *rs)
{
    ...
    if (rs->sock != RADIO_INVALID_SOCKET && !rs->socketClosed) {
        ...
        RADIO_DBG(printf("BEFORE CloseSocket session=%lu fd=%ld\n", rs->session_id, (long)rs->sock););
        rs->socket_close_count++;
        radio_close_socket(rs->sock);
        RADIO_DBG(printf("AFTER CloseSocket session=%lu fd=%ld\n", rs->session_id, closing_fd););
        rs->sock = RADIO_INVALID_SOCKET;
        rs->socketClosed = 1;
...
void Radio_NetworkShutdown(void)
{
    ...
    if (Radio_IsTlsPoisoned() || radio_tls_shutdown_quarantine) {
        /* Only hard AmiSSL/memory poison should reach this path now.
         * Abandon the shared instance rather than walking possibly corrupted
         * AmiSSL state during app shutdown. */
        if (Radio_IsTlsPoisoned())
            printf("APP_CLOSE: AmiSSL poisoned, skipping final AmiSSL shutdown\n");
        else
            printf("APP_CLOSE: AmiSSL shutdown quarantined, skipping final AmiSSL shutdown\n");
...
int Radio_Pump(RadioStream *rs)
{
    ...
    if (rs->isSSL && rs->ssl) {
        ...
        if (n <= 0) {
            ...
            if (e == SSL_ERROR_WANT_READ || e == SSL_ERROR_WANT_WRITE) wb = 1;
                 ...
                 * quarantined and this session is not poisoned. Keep a
                 * diagnostic flag, but cleanup still follows the child-worker
                 * repro and frees SSL/SSL_CTX. */
                rs->sslReadCloseSeen = 1;
                RADIO_DBG(printf("radio-ssl-read: session=%lu SSL_ERROR_ZERO_RETURN seen -- clean TLS EOF\n", rs->session_id));
            }
            else {
                ...
                if (radio_ssl_error_is_fatal(e)) {
                     ...
                     * exactly as fatal. Quarantine HTTPS for the rest of this
                     * app run after reporting the first useful TLS error, but
                     * let this child clean up in the same order as the
                     * standalone repro. */
                    const char *reason = (e == SSL_ERROR_SYSCALL && ssl_lib_error == 0) ?
                    ...
                    if (!rs->tlsFaultCounted) {
                        ...
                        RADIO_DBG(printf("radio-tls: repeat TLS fault in session=%lu (already counted) -- cleanup will still run\n", rs->session_id));
                    }
                    ERR_clear_error();
                    RADIO_DBG(printf("radio-ssl-read: session=%lu TLS fault reported -- repro-order cleanup will run\n", rs->session_id));
                }
            }
        }
    } else
    ...
    if (n < 0 && wb) {
        ...
        if (rs->everPlayed && ++rs->stallPumps >= RADIO_STALL_TIMEOUT_PUMPS) {
             ...
             * file's AmiSSL lifecycle comments exist for. The user can
             * restart the stream from a clean child-owned session with one
             * click instead. */
