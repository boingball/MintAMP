...
static int radio_is_terminal(const RadioStream *rs)
{
    return !rs || rs->status == RADIO_STATUS_CLOSED ||
        rs->status == RADIO_STATUS_ERROR || rs->status == RADIO_STATUS_STOPPING;
}

static int radio_is_stopping(const RadioStream *rs)
{
    return !rs || rs->stopping || rs->status == RADIO_STATUS_STOPPING ||
...
static int radio_send_all(RadioStream *rs, const char *buf, int len)
{
    ...
    while (sent < len && tries < 150) {
        ...
        if (rs->tls) {
            if (!rs->ssl)
                return -1;
            r = SSL_write(rs->ssl, (char *)buf + sent, len - sent);
        } else
#endif
        r = (int)send(rs->sock, (char *)buf + sent, len - sent, 0);
        if (r > 0) { sent += r; continue; }
#if defined(AMIGA_M68K) && defined(ENABLE_AMISSL)
        if (r < 0 && rs->tls && rs->ssl) { int e=SSL_get_error(rs->ssl,r); if(e==SSL_ERROR_WANT_READ||e==SSL_ERROR_WANT_WRITE){ radio_backoff_sleep(); tries++; continue; } }
        else
#endif
...
static int parse_url(RadioStream *rs,const char *url){ const char *p,*slash,*colon; int hl; if(!url) return -1; if(!strncmp(url,"https://",8)){
return -1;
} else if(!strncmp(url,"http://",7)){ rs->tls=0; p=url+7; rs->port=80; } else return -1; slash=strchr(p,'/'); if(!slash) slash=p+strlen(p); colon=memchr(p,':',(size_t)(slash-p)); hl=(int)((colon?colon:slash)-p); if(hl<=0||hl>=(int)sizeof(rs->host)) return -1; memcpy(rs->host,p,hl); rs->host[hl]=0; if(colon) rs->port=atoi(colon+1); if(rs->port<=0)rs->port=rs->tls?443:80; radio_copy_string(rs->path,sizeof(rs->path),*slash?slash:"/"); radio_copy_string(rs->url,sizeof(rs->url),url); return 0; }
...
static void close_current_socket(RadioStream *rs)
{
    if (!rs)
        return;
#if defined(AMIGA_M68K) && defined(ENABLE_AMISSL)
    if (rs->ssl) { SSL_free(rs->ssl); rs->ssl = NULL; }
    if (rs->sslCtx) { SSL_CTX_free(rs->sslCtx); rs->sslCtx = NULL; }
#endif
...
static int reconnect_http(RadioStream *rs)
{
    ...
    if (!rs || rs->stopping || rs->status == RADIO_STATUS_STOPPING) { if (rs) rs->status = RADIO_STATUS_CLOSED; return -1; }
    if (rs->status == RADIO_STATUS_CLOSED)
        rs->status = RADIO_STATUS_RECONNECTING;
...
static void parse_headers(RadioStream *rs,char *h){ char *line=strtok(h,"\r\n"); int code=0; if(line && ci_starts(line,"ICY")) code=200; else if(line && ci_starts(line,"HTTP/")) sscanf(line,"HTTP/%*s %d",&code); else { set_error(rs,"invalid HTTP stream response"); RADIO_OPEN_DEBUG_PRINTF(("radio-open: HTTP header failed\n")); return; } if(code<200||code>299){ set_error(rs,"HTTP stream returned non-success status"); RADIO_OPEN_DEBUG_PRINTF(("radio-open: HTTP header failed status %d\n", code)); return; } while((line=strtok(NULL,"\r\n"))){ char *v=strchr(line,':'); if(!v) continue; *v++=0; line=trim(line); v=trim(v); if(ci_equals(line,"Content-Type")) radio_copy_string(rs->contentType,sizeof(rs->contentType),v); else if(ci_equals(line,"icy-metaint")){ rs->metaint=atoi(v); rs->audioUntilMeta=rs->metaint; } else if(ci_equals(line,"icy-br")) rs->bitrate=atoi(v); else if(ci_equals(line,"icy-name")) radio_copy_string(rs->stationName,sizeof(rs->stationName),v); else if(ci_equals(line,"icy-genre")) radio_copy_string(rs->genre,sizeof(rs->genre),v); else if(ci_equals(line,"icy-url")) radio_copy_string(rs->streamUrl,sizeof(rs->streamUrl),v); else if(ci_equals(line,"Transfer-Encoding") && ci_starts(v,"chunked")) rs->chunked=1; } if(rs->contentType[0] &&
    ...
    !ci_starts(rs->contentType,"audio/flac") &&
    !ci_starts(rs->contentType,"audio/x-flac")) {
    set_error(rs,"unsupported stream Content-Type (expected MPEG, AAC, or FLAC audio)");
    RADIO_OPEN_DEBUG_PRINTF(("radio-open: content type unsupported: %s\n", rs->contentType));
} }
...
int Radio_Pump(RadioStream *rs)
{
    ...
    int n;
    if(!rs) return -1;
    if(radio_is_terminal(rs)) return 0;
    ...
    if (rs->tls) {
        ...
        n=SSL_read(rs->ssl,(char*)b,sizeof(b));
    } else
#endif
    n=(int)recv(rs->sock,(char*)b,sizeof(b),0);
    if(radio_is_stopping(rs)) { close_current_socket(rs); rs->status=RADIO_STATUS_CLOSED; return 0; }
#if defined(AMIGA_M68K) && defined(ENABLE_AMISSL)
    if(n<0 && rs->tls && rs->ssl) { int e=SSL_get_error(rs->ssl,n); if(e==SSL_ERROR_WANT_READ||e==SSL_ERROR_WANT_WRITE){ radio_backoff_sleep(); return 0; } }
    else
#endif
    ...
    if(rs->headerDone && rs->used >= RADIO_START_THRESHOLD) {
        ...
        rs->reconnectDelay = 0;
        rs->everPlayed = 1;
        rs->status=RADIO_STATUS_PLAYING;
    } else if(rs->headerDone && rs->status!=RADIO_STATUS_PLAYING) set_status(rs,RADIO_STATUS_BUFFERING);
    return n;
}
