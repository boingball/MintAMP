/* AmiSSL parent/child concurrency repro -- LOCKED variant.
 *
 * amissl_spawn_repro.c proved that a parent task probing HTTPS and a
 * CreateNewProcTags() child streaming HTTPS, both reusing the same global
 * SocketBase/AmiSSLBase/AmiSSL_ErrNoPtr with NO serialization, can leave the
 * child permanently wedged (observed: stuck at the DNS-resolution stage,
 * never advancing past attempt 1, for 279+ parent loops while the parent
 * kept completing full connect/handshake/read/free cycles on the same
 * shared base).
 *
 * This variant is byte-for-byte the same harness except every touch of that
 * shared state -- the child's InitAmiSSL()/CleanupAmiSSL() pair, and the
 * entire connect/handshake/read/free sequence in stream_once() for BOTH
 * roles -- is wrapped in a real, blocking ObtainSemaphore()/ReleaseSemaphore()
 * pair (not a best-effort/timed-out attempt). Only one task can ever be
 * inside those calls at a time.
 *
 * If the permanent wedge disappears here, that is strong confirmation the
 * root cause is unserialized concurrent access to the shared bsdsocket.library
 * base/AmiSSL instance -- which points the real-app fix at making
 * radio_stream.c's Radio_AmiSslLock() (currently a bounded, give-up-and-
 * proceed-unlocked AttemptSemaphore()) into a real blocking lock, rather than
 * at SSL_free() itself.
 *
 * Build:
 *   make -f Makefile.amiga amissl-spawn-repro-locked-test RADIO=1 SSL=1
 *
 * Run (same env vars as amissl_spawn_repro.c):
 *   amissl_spawn_repro_locked
 ...
 *   amissl_spawn_repro_locked
 *
 * Compare directly against an unlocked amissl_spawn_repro.c soak run with the
 * same env vars: same station churn, same read/iteration targets, only the
 * locking differs.
 */

#if !defined(AMIGA_M68K) || !defined(HAVE_AMISSL)
#include <stdio.h>
int main(void) { puts("amissl_spawn_repro_locked must be built for AmigaOS with HAVE_AMISSL"); return 1; }
#else
...
#define CHILD_STAGE_WAIT_LOCK 13
...
/* Real blocking lock serializing every task's use of the shared
 * SocketBase/AmiSSLBase/AmiSSL_ErrNoPtr state below -- the thing
 * amissl_spawn_repro.c deliberately does NOT have. */
static struct SignalSemaphore g_shared_lock;
...
static const char *child_stage_name(int stage)
{
    switch (stage) {
        ...
        case CHILD_STAGE_WAIT_LOCK: return "wait-lock";
    }
    return "unknown";
}
...
/* AmigaOS has no Unix-style envp handed to a process at startup --
 * variables live in ENV: and are read via dos.library's GetVar(). Several
 * cross-toolchain libc's getenv() never bridges that (always returns NULL),
 * which silently looks like "the env var had no effect" instead of an
 * error, so read it directly here rather than trust getenv(). Flags=0
 * checks local (Set) variables first, then global (SetEnv/ENV:) ones. */
static int env_int(const char *name, int default_val)
{
    ...
    len = GetVar((STRPTR)name, (STRPTR)buf, sizeof(buf) - 1, 0);
...
static int send_request(const char *role, SSL *ssl, const char *host, const char *path)
{
    ...
    n = snprintf(req, sizeof(req),
        "GET %s HTTP/1.1\r\n"
        "Host: %s\r\n"
        "User-Agent: BoingPlayer-amissl-spawn-repro-locked/0.1 AmigaOS\r\n"
        "Icy-MetaData: 1\r\n"
        "Accept: */*\r\n"
        "Connection: close\r\n\r\n",
        path, host);
...
/* Everything from here to the matching ReleaseSemaphore() runs with the
 * shared SocketBase/AmiSSLBase/ErrNoPtr state locked to this task only --
 * this is the one structural difference from amissl_spawn_repro.c. */
static int stream_once(const char *role, const Station *st, int attempt, int reads)
{
    ...
    set_child_stage(role, CHILD_STAGE_WAIT_LOCK);
    ObtainSemaphore(&g_shared_lock);

    sock = tcp_connect_polling(role, st->host, st->port);
    if (sock == -1) { ReleaseSemaphore(&g_shared_lock); return g_stop_requested ? 0 : -1; }
...
cleanup:
    ...
    ReleaseSemaphore(&g_shared_lock);
    return rc;
}
...
static void child_entry(void)
{
    ...
    g_child_started = 1;
    if (g_child_log) {
        progress("child: InitAmiSSL using inherited SocketBase/shared ErrNoPtr (locked)");
    ...
    set_child_stage("child", CHILD_STAGE_WAIT_LOCK);
    ObtainSemaphore(&g_shared_lock);
...
int main(void)
{
    ...
    progress("AmiSSL parent/child shared SocketBase concurrency repro -- LOCKED variant");
    InitSemaphore(&g_shared_lock);
    load_config();
    ...
    child = CreateNewProcTags(NP_Entry, (ULONG)child_entry,
                              NP_Name, (ULONG)"amissl_spawn_child_locked",
                              NP_StackSize, 65536,
                              NP_Priority, 0,
                              TAG_DONE);
